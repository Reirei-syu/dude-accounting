...
- 优先使用 `--payload-file` 或 `--payload-stdin`
...
- `voucher next-number` 与新建凭证自动取号只统计有效凭证；删除态凭证保留历史编号但不继续占用新号。
- `voucher renumber` 只整理有效凭证号；删除态凭证保留历史编号，如恢复删除时编号已被有效凭证占用，系统会拒绝恢复并提示先整理或另行处理。
...
1. 查询现状
2. 准备 payload 文件或通过标准输入传 payload
3. 执行命令
4. 检查返回值中的 `status`
