...
describe('pl carry forward service', () => {
  ...
  it('previews NPO carry-forward entries against the correct net-asset targets', () => {
    ...
    seedSubject(db, 2, '410101', '捐赠收入-非限定性', 'profit_loss', -1)
    seedSubject(db, 2, '410102', '捐赠收入-限定性', 'profit_loss', -1)
    seedSubject(db, 2, '5101', '业务活动成本', 'profit_loss', 1)
    seedSubject(db, 2, '3101', '非限定性净资产', 'equity', -1)
    seedSubject(db, 2, '3102', '限定性净资产', 'equity', -1)
