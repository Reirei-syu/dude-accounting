    ...
    (
      ...
      try {
        ...
        if (Number.isNaN(startYear) || Number.isNaN(targetYear)) {
          return { success: false, error: '期初期间或账套启用年月格式不正确' }
        }
        if (targetYear < startYear) {
          return { success: false, error: '期初期间不得早于账套启用年份' }
        }
        ...
        if (period !== allowedPeriod) {
          return {
            success: false,
            error: '期初期间需与账套启用年月一致，后续年度仅允许 1 月'
          }
        }
        if (!Array.isArray(entries)) {
          return { success: false, error: '期初数据格式不正确' }
        }
        ...
        return {
          success: false,
          error: error instanceof Error ? error.message : '保存期初余额失败'
        }
      }
    }
  )
