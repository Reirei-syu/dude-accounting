...
- 若检测到历史错误版本曾将数据库保存在旧安装目录 `data` 或 legacy `userData` 目录，则仅在 `userData/data` 尚不存在数据库时执行一次性兼容迁移；若两处旧库同时存在，则按数据库主文件及其 `-wal/-shm` 辅助文件中的最新修改时间选择更新的一份作为迁移源
...
管理员账号维护（包括新建用户、维护权限、修改已有账号密码）；其中已有账号的密码修改仅允许 `admin` 执行，`admin` 也可修改自己的密码，且密码允许设为空。
...
当前还承载用户级界面个性化偏好，包括默认账套、默认首页和自定义壁纸；系统参数页同时补充展示错误日志目录与今日日志状态，便于运维排障。
...
- 会计科目设置页的科目列表新增“现金流量”列；当上级科目已标记为现金流量科目时，下级科目在新增和编辑时均必须保持为现金流量科目，且父级改为现金流量科目时会向全部下级级联同步该标记。
...
- 正在将备份主流程从“系统级数据库快照包”重构为“账套级备份包 + 导入为新账套”；历史整库快照仅保留 legacy 校验/删除能力。
- 备份创建支持用户自定义保存目录，并记忆最近一次使用的备份目录。
- 恢复既支持按系统内备份记录恢复，也支持从自选备份包目录直接发起恢复。
- 备份时间选择器应允许选择账套全部期间，并优先默认指向当前期间；若账套暂无期间，也允许按“未设置期间”创建账套备份包。
...
- `print:*`
- `backup:create/list/validate/delete/import`
...
- 电子凭证处理必须具备重复入账拦截能力。
- 账套删除前的备份与电子档案导出改为强提示，不再做硬阻断；但必须要求显式风险确认并将风险快照写入操作日志。
- 备份文件与档案导出文件不得混用。
...
- 备份与归档时间选择器必须采用选择方式；备份应允许选择全部期间并优先默认当前期间，归档默认选中最近一次可归档年度。
- 金额按“分”存储，输入允许两位小数。
...
- 操作日志查询与导出尚未完成。
- 账套备份正从系统级数据库快照包切换为账套级备份包；本轮目标是完成账套级包生成、校验与导入为新账套，legacy 整库快照不再作为主流程。
- 已记账凭证控制需要从当前实现继续收紧。
